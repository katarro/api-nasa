				...
				<form onSubmit={handleSubmit(onSubmit)}>
				 ...
				 {loading ? (
					...
					<table className="table">
						...
						<tbody>
							{asteroids[initial_date.year + '-' + initial_date.month + '-' + initial_date.day].map(
								(i) => (
									<tr id="tr" key={i.neo_reference_id}>
										...
										{i.close_approach_data.map((j) => (
											<td>
												{Number.parseFloat(j.relative_velocity.kilometers_per_hour).toFixed()}{' '}
												Km/h
											</td>
										))}
										...
										<td>
											...
											{i.estimated_diameter.meters.estimated_diameter_max.toFixed()} Mts
										</td>
										<td>{i.is_potentially_hazardous_asteroid + ''}</td>

										{i.close_approach_data.map((j) => (
											<td> {Number.parseInt(j.miss_distance.kilometers).toFixed()} Km </td>
										))}
									</tr>
								)
							)}
